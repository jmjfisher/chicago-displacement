...
function createChart(){
    ...
    */
    ...
    //use queue to parallelize asynchronous data loading
    d3.queue()
        //.defer(d3.csv, "data/master.csv") //asycn load attributes from masterCSV
        .defer(d3.csv, "cars.csv") //asycn load attributes from masterCSV
        .await(callback);
        
    function callback (error, csvMaster) {
        console.log("Master CSV, primary key = CG_GEOID",csvMaster)
        setChart(csvMaster);
    }
    ...
    function setChart(csvMaster){
      ...
      g.append("g")
          .attr("class", "brush")
          .each(function(d) {
            d3.select(this).call(yScale[d].brush = d3.brush().y(yScale[d]).on("brushstart", brushstart).on("brush", brush));
          })
        .selectAll("rect")
          .attr("xScale ", -8)
          .attr("width", 16);
...
>>>>>>> d63f9631a4e7750ead7c0e2532f5f085170bc3d6
